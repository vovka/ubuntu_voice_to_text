...
def test_audio_processor_with_output_dispatcher():
    """Test AudioProcessor integration with OutputDispatcher."""
    try:
        ...
        # Create output dispatcher with test target
        dispatcher = OutputDispatcher()
        dispatcher.initialize()
        ...
        # Create AudioProcessor with custom dispatcher
        # Note: This may skip if recognition dependencies are not available
        try:
            audio_processor = AudioProcessor(config, state_ref, output_dispatcher=dispatcher)
            assert audio_processor.output_dispatcher is dispatcher
            print("AudioProcessor integration with OutputDispatcher works")
        except SystemExit:
            pytest.skip("Recognition source not available for testing")
            
    except ImportError as e:
        pytest.skip(f"Skipping due to missing dependencies: {e}")


def test_pipeline_voice_typing_with_output_dispatcher():
    """Test PipelineVoiceTyping integration with OutputDispatcher."""
    try:
        ...
        # Create mock tray icon manager
        tray_icon_manager = Mock()
        ...
        # Create PipelineVoiceTyping with custom dispatcher
        try:
            pipeline_vt = PipelineVoiceTyping(
                config, state_ref, tray_icon_manager, 
                output_dispatcher=dispatcher
            )
            assert pipeline_vt.output_dispatcher is dispatcher
            print("PipelineVoiceTyping integration with OutputDispatcher works")
