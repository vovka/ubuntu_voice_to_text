...
class TrayIconManager:
    ...
    def create_image_text(self, state='idle'):
        ...
        dc.ellipse([(4, 4), (width-4, height-4)], fill=color, outline=(60, 60, 60), width=2)
...
class HotkeyManager:
    ...
    def on_press(self, key):
        ...
        if self.state_ref.current_keys == self.config.HOTKEY_COMBO and self.state_ref.state != 'listening':
    ...
    def on_release(self, key):
        ...
        if self.state_ref.state == 'listening' and not self.state_ref.current_keys.issuperset(self.config.HOTKEY_COMBO):
