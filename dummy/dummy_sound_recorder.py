        ...
        try:
            while self._running:
                try:
                    ...
                    logger.info(f"DummySoundRecorder received control command: {control_command}")
                    ...
                    logger.info(f"DummySoundRecorder is alive - recording: {self._recording}")
                    
                    if self._recording:
                        # Simulate audio chunk
                        audio_chunk = f"dummy_audio_chunk_{asyncio.get_event_loop().time()}"
                        await self._audio_output_queue.put(audio_chunk)
                        logger.info(f"DummySoundRecorder put audio chunk: {audio_chunk}")
