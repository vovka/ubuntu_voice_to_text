...
class DummyKeyboardListener(IKeyboardListener):
    ...
    def __init__(
        ...
        self._running = False
        logger.info(
            f"DummyKeyboardListener initialized with config: {self._config}, using output_queue: {id(self._output_queue)}"
        )
        ...
        try:
            while self._running:
                ...
                if self._running:
                    ...
                    logger.info(
                        f"DummyKeyboardListener detected hotkey: {hotkey_event}"
                    )
